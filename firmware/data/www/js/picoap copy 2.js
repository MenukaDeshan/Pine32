...
async function triggerScan(btn) {
  ...
  try {
    // Start the scan
    const startRes = await fetch('/scan');
    if (!startRes.ok) throw new Error('Scan start failed: ' + startRes.status);
    ...
    const pollResults = async () => {
      attempts++;
      const resultsRes = await fetch('/scan/results');
    ...
    lastScanResults = Array.isArray(scanResults) ? scanResults : [];
...
function updatePineSummary() {
  try {
    const ssidCount = lastScanResults ? lastScanResults.length : 0;
    ...
    // Update the recent APs display
    const recent = document.getElementById('pineRecent');
    if (recent) {
      if (lastScanResults && lastScanResults.length > 0) {
        let html = '';
        lastScanResults.slice(0, 5).forEach(ap => {
          // Check if ap exists and has properties
          if (ap) {
            const ssid = escapeHtml(ap.ssid || 'Hidden');
            const rssi = escapeHtml(ap.rssi !== undefined ? ap.rssi : '—');
            html += `<div>${ssid} (${rssi} dBm)</div>`;
          }
        });
        
        if (lastScanResults.length > 5) {
          html += `<div class="muted-small">... and ${lastScanResults.length - 5} more</div>`;
        }
        recent.innerHTML = html;
      } else {
        recent.innerHTML = '<p>No APs scanned yet. Click <b>Scan Wi‑Fi</b>.</p>';
      }
    }
  } catch (error) {
    console.error('Error in updatePineSummary:', error);
    
    // Fallback updates
    const totalEl = document.getElementById('pine_total_ssids');
    if (totalEl) totalEl.textContent = 'SSIDs: 0';
    
    const recent = document.getElementById('pineRecent');
    if (recent) recent.innerHTML = '<p class="muted-small">Error loading scan results</p>';
  }
}
...
async function loadClients() {
  ...
  try {
    ...
    cont.innerHTML = '<p class="muted-small">Clients endpoint not available.</p>';
  }
}
...
// Debug line to verify the file is loaded
console.log('picoap.js loaded successfully - all functions available');